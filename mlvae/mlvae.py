...
def validate_labels(feature_dict, class_sizes):
    for k in feature_dict:
        with tf.control_dependencies([tf.assert_greater(class_sizes[k],
                                                        tf.reduce_max(
                                                                feature_dict[
                                                                    k]))]):
            pass
...
def preoutput_MLP(inputs, embed_dim, num_layers=2, activation=tf.nn.elu):
    ...
    for i in range(num_layers):
        x = dense_layer(x, embed_dim, 'l{}'.format(i + 1),
...
def MLP_gaussian_posterior(inputs, embed_dim, latent_dim, min_var=0.0):
    ...
    zv = dense_layer(x, latent_dim, 'zv',
                     tf.nn.softplus)  # variance must be positive
    if min_var > 0.0:
...
class MultiLabel(object):
    def __init__(self,
                 ...
                 # x's that are being reconstructed; keys in a dictinary corresponding to input batches
                 hp=None):
        ...
        for k, v in class_sizes.items():
            self._qy_templates[k] = tf.make_template('qy_{}'.format(k),
                                                     MLP_unnormalized_log_categorical,
                                                     output_size=v,
                                                     embed_dim=self._hp.embed_dim)
        self._qz_template = tf.make_template('qz', MLP_gaussian_posterior,
                                             embed_dim=self._hp.embed_dim,
                                             latent_dim=self._hp.latent_dim,
                                             min_var=self._hp.min_var)
    ...
    def get_label_log_probability(self, feature_dict, features, z,
        ...
        if distribution_type == 'p':
            ...
            raise ValueError(
                'unrecognized distribution type: %s' % (distribution_type))
        log_dist = tf.nn.log_softmax(logits)
        log_dist = tf.reshape(log_dist, [self._batch_size, self._class_sizes[
            feature_name]])  # reshape to ignore first dimension

        r = tf.range(0, self._batch_size, 1)
        r = tf.expand_dims(r, axis=0)

        label_idx = tf.expand_dims(label_idx, axis=0)
        label_idx = tf.concat([tf.transpose(r), tf.transpose(label_idx)],
                              axis=1)

        log_probs = tf.gather_nd(log_dist,
                                 label_idx)  # get the feature_dict[feature_name][i]'th element from log_dist[i] (in batch mode)
        return log_probs

    def get_label_instantiation(self, features, z, feature_dict,
                                observed_dict):
        ...
        for k in feature_dict:
            if observed_dict[k] is True:
                instantiation[k] = tf.one_hot(feature_dict[k],
                                              self._class_sizes[k])  # one-hot
            else:
                # sample a value of y_k
                qy_logits = self._qy_templates[k](features + [z])
                instantiation[k] = self.sample_y(qy_logits, k,
                                                 argmax=False)  # approx one-hot
        return instantiation

    def get_Eq_log_pz(self, zm, zv, zm_prior, zv_prior, z_samples=None):
        res = 0
        if z_samples is None:
            z_samples = [gaussian_sample(zm, zv) for _ in
                         range(self._hp.num_z_samples)]
        for z in z_samples:
    ...
    def get_kl_qp(self, features, feature_name, feature_dict, observed_dict,
                  zm, zv, z_samples=None):
        kl_qp = 0
        if observed_dict[feature_name] is True:
            ...
            kl_qp += self.get_Eq_log_qy(feature_name, zm, zv,
                                        z_samples=z_samples) - self.get_Eq_log_py(
                features, feature_name, feature_dict, observed_dict, zm, zv,
                z_samples=z_samples)
        else:
            if self._hp.expectation == 'exact':
                ...
                if z_samples is None:
                    z_samples = [gaussian_sample(zm, zv) for _ in
                                 range(self._hp.num_z_samples)]
                for z in z_samples:
                    qy_logits = self._qy_templates[feature_name](
                        features + [z])
                    qcat = Categorical(logits=qy_logits,
                                       name='qy_{}_{}_cat'.format(feature_name,
                                                                  i))

                    py_logits = self._py_templates[feature_name](z)
                    pcat = Categorical(logits=py_logits,
                                       name='py_{}_{}_cat'.format(feature_name,
                                                                  i))

                    kl = kl_divergence(qcat, pcat)
                    res += kl
                kl_qp += res / len(
                    z_samples)  # average KL divergence between q and p for feature k
            elif self._hp.expectation == 'sample':
                # Decouples into q and p terms (linearity of expectation), so we calculate the q and p terms separately
                kl_qp += self.get_Eq_log_qy(feature_name, zm, zv,
                                            z_samples=z_samples) - self.get_Eq_log_py(
                    features, feature_name, feature_dict, observed_dict, zm,
                    zv, z_samples=z_samples)
            else:
                raise ValueError('unrecognized expectation mode: %s' % (
                self._hp.expectation))

        return kl_qp

    def get_Eq_log_py(self, features, feature_name, feature_dict,
                      observed_dict, zm, zv, z_samples=None):
        Eq_log_py = 0
        if observed_dict[feature_name] is True:
            # feature is observed (in all examples in this batch)
            res_p = 0
            if z_samples is None:
                z_samples = [gaussian_sample(zm, zv) for _ in
                             range(self._hp.num_z_samples)]
            for z in z_samples:
                log_probs = self.get_label_log_probability(features, z,
                                                           feature_name,
                                                           feature_dict[
                                                               feature_name],
                                                           distribution_type='p')
            ...
            if self._hp.expectation == 'exact':
                ...
                raise ValueError(
                    'sample expectation mode not supported: %s' % (
                    self._hp.expectation))
            else:
                raise ValueError('unrecognized expectation mode: %s' % (
                self._hp.expectation))
        ...
        if observed_dict[feature_name] is True:
            ...
            if self._hp.expectation == 'exact':
                ...
                for z in z_samples:
                    ...
                    qy_concrete = ExpRelaxedOneHotCategorical(self._tau,
                                                              logits=qy_logits,
                                                              # logits do *not* need to be manually exp-normalized for this distribution (the distribution automatically exp-normalizes)
                                                              name='qy_{}_{}_concrete'.format(
                                                                  feature_name,
                                                                  i))
                    for _ in range(self._hp.num_y_samples):
                        y_sample = tf.exp(
                            qy_concrete.sample())  # each row is a continuous approximation to a categorical one-hot vector over label values
                        ...
                        res += qy_concrete.log_prob(
                            y_sample)  # log q(y_sample)
    ...
    def get_Eq_log_px(self, targets, features, feature_dict, observed_dict, zm,
                      zv, z_samples=None):
        res = 0
        if z_samples is None:
            z_samples = [gaussian_sample(zm, zv) for _ in
        ...
        for z in z_samples:
            for _ in range(self._hp.num_y_samples):
                instantiation = self.get_label_instantiation(features, z,
                                                             feature_dict,
                                                             observed_dict)
                # instantiation dict to list with canonical ordering
                instantiation_list = [instantiation[k] for k in
                                      self._dataset_order]  # dataset_order: consistent ordering of tasks/datasets
                markov_blanket = tf.concat([instantiation_list],
                                           axis=1)  # parents of x in p model
                _feature_name = set(
                    f for f in observed_dict if observed_dict[f] is True)
                assert len(_feature_name) == 1
                nll = self.decode(targets, markov_blanket,
                                  _feature_name)  # reconstruction loss
                res += nll
        Eq_log_px = res / (len(z_samples) * self._hp.num_y_samples)

    def get_disc_loss(self, features, feature_dict, feature_name,
                      observed_dict, zm, zv, z_samples=None):
        disc_loss = 0
        if observed_dict[feature_name] is True:
            res = 0
            if z_samples is None:
                z_samples = [gaussian_sample(zm, zv) for _ in
                             range(self._hp.num_z_samples)]
            for z in z_samples:
                res += self.get_label_log_probability(feature_dict, features,
                                                      z, feature_name,
                                                      feature_dict[
                                                          feature_name],
                                                      distribution_type='q')
    ...
    def get_multi_task_loss(self, dataset_batches):
        ...
        for dataset_name, batch in dataset_batches.items():
            ...
            if self._hp.loss_combination == "even":
                total_loss += self.get_loss(feature_dict, inputs=inputs,
                                            targets=targets,
                                            loss_type=self._hp.loss_type,
                                            features=None)
            else:
                raise ValueError("unrecognized loss combination type: %s" % (
                self._hp.loss_combination))

        return total_loss

    def get_total_discriminative_loss(self, features, feature_dict,
                                      observed_dict, zm, zv, z_samples=None):
        total_disc_loss = 0
        for k in feature_dict:
            total_disc_loss += self.get_disc_loss(features, feature_dict, k,
                                                  observed_dict, zm, zv,
                                                  z_samples=z_samples)

        return total_disc_loss

    def get_total_generative_loss(self, features, feature_dict, observed_dict,
                                  targets, zm, zv, z_samples=None):
        ...
        total_kl_qp = 0
        for k in feature_dict:
            total_kl_qp += self.get_kl_qp(features, k, feature_dict,
                                          observed_dict, zm, zv,
                                          z_samples=z_samples)
        Eq_log_px = self.get_Eq_log_px(targets, features, feature_dict,
                                       observed_dict, zm, zv,
                                       z_samples=z_samples)
    ...
    def get_loss(self,
        ...
        if features is None:
            ...
            for _f in _feature_name:
                features = self.encode(inputs,
                                       _f)  # encode with encoder corresponding to observed feature
        ...
        if loss_type == 'discriminative':
            # Discriminative loss
            d_loss = self.get_total_discriminative_loss(features, feature_dict,
                                                        observed_dict, zm, zv,
                                                        z_samples=z_samples)
            ...
            # Generative loss
            g_loss = self.get_total_generative_loss(features, feature_dict,
                                                    observed_dict, targets, zm,
                                                    zv, z_samples=z_samples)
