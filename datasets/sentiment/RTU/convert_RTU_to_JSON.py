...
"""Convert original RTU files to json format"""
...
with open(os.path.join(path, 'rtu/rt_user.neg')) as file:
...
with open(os.path.join(path, 'rtu/rt_user.pos')) as file:
    for line in file.readlines():
        train_list.append(
            {'index': index,
             'text': line.strip(),
...
def next_3_lines(file):
    return [line.strip() for line in islice(file, 3)]


with open(os.path.join(path, 'rtu/s.user.movie.2k.human.test')) as file:
    lines = next_3_lines(file)
    while lines:
        test_list.append({
            'index': index,
            'text': lines[0],
