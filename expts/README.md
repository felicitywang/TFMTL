...
All the example commands can be found in `expts/example/`. (Run `python get_encoders.py` or `python get_qsub_encod.py qsub_config_encod.json` to generate `encoders.json` or `qsub_config_encod.json`.)
...
Or you could run `python qsub_mtl_jobs.py config_json_name encod_json_name` to generate running scripts in organized directories.
- `qsub_config_expt.json` is an example of `config_json_name`; you may want to read it carefully and modify some arguments before using it.
- `qsub_config_encod.json` generated by `python get_qsub_encod.py qsub_config_encod.json` is an example of `encod_json_name`. Names and architectures for difference encoders can be easily changed in `get_qsub_encod.py`.
