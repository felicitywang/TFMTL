...
if debug:
    ...
    root_dir = "/export/a08/fwang/tfmtl/expts/paragram_pretrain/"
    data_dir = "/export/a08/fwang/tfmtl/expts/all_EMNLP/data/tf/"
    encoder_dir = "/export/a08/fwang/tfmtl/expts/paragram_pretrain/encoder_files/"
    # code_path = "/export/a08/fwang/tfmtl/expts/scripts/discriminative_driver.py"
    code_path = "/export/a08/fwang/tfmtl/expts/scripts/test_FNC.py"
...
def make_job(dataset, vocab, task, init_or_expand, finetune_or_freeze,
    ...
    name = dataset + '_' + task + '_' + vocab + \
           '_' + init_or_expand + '_' + finetune_or_freeze + '_' + str(
        random_seed)

    result_dir = os.path.join(root_dir, 'early_results', dataset, name)
    make_dir(result_dir)

    job_dir = os.path.join(root_dir, 'test_jobs', dataset)
    make_dir(job_dir)
    job_name = 'test_' + name + '.sh'
...
#$ -l \'hostname=b1[12345678]*|c*,gpu=1,mem_free=3G,ram_free=3G\''\n\
#$ -pe smp 2\n\
#$ -V\n\
#$ -q g.q\n\
\n\
...
\n\
  CUDA_VISIBLE_DEVICES=`free-gpu` /home/fwang/gpu/bin/python3 " + \
             ...
             "  --architecture " + 'serial_paragram_' \
             + init_or_expand + "_" + finetune_or_freeze + " \\\n" + \
             "  --seed " + str(random_seed) + " \\\n" + \
             "  --alphas " + alphas + " \\\n" + \
             "  --log_file " + os.path.join(result_dir, 'test.log') + " \\\n" + \
            ...
            for vocab in ['all_0']:
                ...
                for init_or_expand in ['init']:
                    for finetune_or_freeze in ['finetune']:
                        for random_seed in [42, 21, 11]:
                            make_job(dataset, vocab, task, init_or_expand,
                                     finetune_or_freeze, random_seed)
