...
if debug:
  ...
  root_dir = "/export/a08/fwang/tfmtl/expts/all_EMNLP_27B/"
  ...
  json_dir = "/export/a08/fwang/tfmtl/expts/all_EMNLP_27B/data/json/"
...
def make_job(dataset, vocab, task, init_or_expand, finetune_or_freeze):
  ...
  job_dir = os.path.join(root_dir, 'test_jobs_21', dataset)
  make_dir(job_dir)
  ...
  for task in ['st', 'mt']:
    for dataset in RUDER_TASKS:
      for vocab in ['train_1', 'all_0']:
        # for vocab in ['all_0']:
        for init_or_expand in ['init', 'expand']:
