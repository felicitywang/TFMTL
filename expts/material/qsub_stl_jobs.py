...
def flags_from_params(params, key_prefix='\\\n    --', key_suffix=''):
  pairs = ['{}{} {} {}'.format(key_prefix, key, value, key_suffix)
...
def run_single_experiment(meta_config,
  ...
  qsub_params = {
    'e': os.path.join(results_dir, 'e'),
    'o': os.path.join(results_dir, 'o'),
    'l': 'mem_free={:d}G,ram_free={:d}G'.format(meta_config.mem_ram,
                                                meta_config.mem_ram)
  }
...
def consistent(encoders, exp_comb):
  ...
  if embedders_tied:
    ...
    embed_kwargs_names = set([enc['embed_kwargs_names']
                              for enc in encoders])
  ...
  if extractors_tied:
    ...
    for enc in encoders:
      ...
      extract_kwargs = {e_k_n: enc[e_k_n]
