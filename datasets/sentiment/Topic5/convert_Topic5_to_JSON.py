...
def readTopic5Way(datafolder="./data/", debug=True, num_instances=20):
    topic_based_path = os.path.join(
        datafolder,
        'semeval2016-task4c-topic-based-sentiment')
    train_path = os.path.join(
        topic_based_path,
        '100_topics_100_tweets.topic-five-point.subtask-CE.train.gold_downloaded.tsv')  # NOQA
    dev1_path = os.path.join(
        topic_based_path,
        '100_topics_100_tweets.topic-five-point.subtask-CE.dev.gold_downloaded.tsv')  # NOQA
    dev2_path = os.path.join(
        topic_based_path,
        '100_topics_100_tweets.topic-five-point.subtask-CE.devtest.gold_downloaded.tsv')  # NOQA
    test_data_path = os.path.join(
        topic_based_path,
        'SemEval2016-task4-test.subtask-CE.txt')
    test_labels_path = os.path.join(
        topic_based_path,
        'SemEval2016_task4_subtaskC_test_gold.txt')
    ...
    assert data_train["labels"] == sorted(list(REMAPPED_VALUES.values()))
...
def parse_topic_based(file_path, debug=False, num_instances=20):
    ...
    with open(file_path) as f:
        for i, line in enumerate(f):
            ...
            data["stance"].append(REMAPPED_VALUES[sentiment])
...
def make_example_list(d, starting_index):
    ...
    for example in examples:
        ...
        ex['seq2'] = example[1]
        label = example[2]
        if label not in REMAPPED_VALUES.values():
...
if __name__ == "__main__":
    ...
    data_train, data_dev1, data_test = readTopic5Way(datafolder=datafolder,
                                                     debug=False,
                                                     num_instances=20)
