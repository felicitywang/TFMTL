...
# TODO and then print the things to paste in google sheets

"""Compute p_miss and p_fa locally for 1S DEV"""
import os
import sys
...
def get_gold_doc_ids(filename):
    """return a dictionary of all the gold doc ids for each domain in DEV 1S"""
    gold_doc_ids = dict()
    all_files = []
...
def get_all_doc_ids():
    all_doc_ids = []
    with open('dev_1S_all_doc_ids.txt') as file:
        for line in file.readlines():
...
def main():
    ...
    # assert os.path.exists(os.path.join(sys.argv[1], '1S.csv'))
