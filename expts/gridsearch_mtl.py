...
'''
        venv: name of anaconda virtual environment
        ...
        total_slots: total number of slots to dedicate to experiments (must not exceed your quota)
...
def dict_append(d1, d2):
    assert not set(d1.keys()).intersection(set(d2.keys())), "can't append overlapping dictionaries: d1={}, d2={}".format(d1, d2)
...
def enumerate_param_combs(tree):
    ...
    for key, value in tree.items():
        assert type(key) in [str], "Found non-str variable key: %s of type %s" % (key, type(key))
        ...
        if type(value) == list:
            ...
            if all(is_dict):
                ...
                raise ValueError("can't have mixed dict and non-dict values here")
            else:
                var_params.append((key, value))
        elif type(value) in [str, float, int, bool] or value is None:
            fixed_params.append((key, value))
        else:
            raise ValueError("Found non-list, non-str variable value: %s : %s" % (key, value))
    ...
    if recursed_product:
        recursed_product = [reduce((lambda x, y: dict_append(x, y)), lst, {}) for lst in recursed_product]
    ...
    for var_param_values_comb in var_param_values_product:
        ...
        for recursed_val in recursed_product:
            arg_dict = dict_append(dict_append(fixed_dict, var_param_comb), recursed_val)
...
def name_from_params(params, keys=['model']):
    ...
    assert all((params.get(key) is not None for key in keys)), "params was missing one of {}".format(keys)
...
def flags_from_params(params, key_prefix='--'):
    pairs = ['{}{} {}'.format(key_prefix, key, value) for key, value in params.items()]
    return ' '.join(pairs)
...
def create_qsub_params(meta_config):
    qsub_params = {
        ...
        'l': 'mem_free={:d}G,ram_free={:d}G'.format(meta_config.mem_ram, meta_config.mem_ram)
        }
...
def complete_path_name(path_name_template, exp_params_comb):
    ...
    if path_name_template.startswith('/') and not new_path_name.startswith('/'):
...
def write_encoder_file(exp_params_comb):
  ...
  datasets = exp_params_comb['expt_setup']['datasets'].split()

  tied_config = exp_params_comb.pop('encoder_tying')
  
  encoder_configs = dict()
  for dataset in datasets:
    encoder_configs[dataset] = exp_params_comb['expt_setup'][dataset]

  configuration = dict()
  configuration[architecture_name] = dict()
  configuration[architecture_name]['embedders_tied'] = tied_config['embedders_tied']
  configuration[architecture_name]['extractors_tied'] = tied_config['extractors_tied']

  for dataset in datasets:
    configuration[architecture_name][dataset] = encoder_configs[dataset]

  if tied_config['embedders_tied']:
    # assert that embedder configs are the same
    assert all([configuration[architecture_name][a]['embed_fn'] == configuration[architecture][b]['embed_fn']
                for a in datasets
                for b in datasets])
    assert all([configuration[architecture_name][a]['embed_kwargs'] == configuration[architecture][b]['embed_kwargs']
                for a in datasets
                for b in datasets])

  if tied_config['extractors_tied']:
    # assert that extractor configs are the same
    assert all([configuration[architecture_name][a]['extract_fn'] == configuration[architecture][b]['extract_fn']
                for a in datasets
                for b in datasets])
    assert all([configuration[architecture_name][a]['extract_kwargs'] == configuration[architecture][b]['extract_kwargs']
                for a in datasets
                for b in datasets])
...
def run_single_experiment(meta_config, exp_params_comb, qsub_params, debug=False):
    # Generate the name for this job, based on params
    print(exp_params_comb)
    #name = name_from_params(exp_params_comb, keys=["expt_setup"])
    name = name_from_params(exp_params_comb, keys=[])
    ...
    results_file = os.path.join(res_dir, '{}.json'.format(name))
    if os.path.exists(results_file):
        # job has already run/started
        print('DONE: {} exists'.format(results_file))
        return False
    else:
        exp_params_comb['output'] = results_file

    if debug:
      print(exp_params_comb)
      return True
        
    encoder_file = write_encoder_file(exp_params_comb)

    # Remove dataset-specific encoder information before writing bash script
    datasets = exp_params_comb['expt_setup']['datasets'].split()
    for dataset in datasets:
      exp_params_comb['expt_setup'].pop(dataset)
...
#      print(embed_kwargs)
#    print('-')
    if not all([d1 == d2 for d1 in all_embed_kwargs for d2 in all_embed_kwargs]):
  ...
  if extractors_tied:
    ...
    extract_kwargs_names = set([enc['extract_kwargs_names'] for enc in encoders])
...
#      print(extract_kwargs)
#    print('--')
    if not all([d1 == d2 for d1 in all_extract_kwargs for d2 in all_extract_kwargs]):
...
def run_all_experiments(meta_config, exp_config, encoder_config, debug=False):
    ...
    # TODO: merge exp_params_combs_list with encoder_params_combs_list
    # and exclude ones that share encoders but have different encoder settings
    exp_params_combs_list = merge_combinations(exp_params_combs_list, encoder_params_combs_list)

    for i in range(10):
      print(exp_params_combs_list[i])
      print('---')
    #for comb in exp_params_combs_list:
    #  print(comb)
    #  print('---')
    #print('==============')
    #for comb in encoder_params_combs_list:
    #  print(comb)
    #  print('---')
    print(len(exp_params_combs_list))
    #print(len(encoder_params_combs_list))
    ...
    jobs_run = 0
    print('STARTING: running up to {} of {} total jobs'.format(jobs_to_run, len(exp_params_combs_list)))
    for exp_params_comb in exp_params_combs_list:
        success = run_single_experiment(meta_config, exp_params_comb, qsub_params, debug=debug)
...
def main():
    if len(sys.argv) < NUM_ARGS + 1:
        print('Usage: python gridsearch_mtl.py <config-file>')
        sys.exit(1)
