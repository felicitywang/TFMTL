...
Running steps(program mode)
    1. train: train the initial model A(need to train A and B both)
    2. test_train: get results from the model trained in 1
    3. transfer: train the new model using labels predicted from the
    previous model
    4. test_transfer: ... in 2
...
flags = tf.app.flags
flags.DEFINE_string('program_mode', "train",
                    "train: train the initial model with text as the only "
                    "input; "
                    "transfer: train a new model using the new features; "
                    "test: test the model")
...
def test_model(model_path, x_test, y_test, pred_test=None,
               program_mode='test_train'):
    with tf.Session() as sess:
        ...
        output_dict = predictor(
            {'x': np.hstack((x_test.toarray(), pred_test))} if
            program_mode == 'test_transfer' else {'x': x_test.toarray()})
...
class MLP(object):
    ...
    def train(self, x_train, y_train, x_dev, y_dev, pred_train=None,
              pred_dev=None, program_mode='train'):
        """ Train

        program_mode == 'train': train the initial model with the text(
        tf-idf) as the only input
        program_mode == 'transfer': train a new model with the output labels from another trained
        model as a new feature

        1. train several epochs on the training set
        2. evaluate on dev set, save results for each time
        3. stop after certain epochs and use the best model(no early stopping)
        ...
        train_input_fn = tf.estimator.inputs.numpy_input_fn(
            x={'x': np.hstack((x_train.toarray(), pred_train))} if
            program_mode == 'transfer' else {'x': x_train.toarray()},
            y=y_train,
        ...
        dev_input_fn = tf.estimator.inputs.numpy_input_fn(
            x={'x': np.hstack((x_dev.toarray(), pred_dev))} if
            program_mode == 'transfer' else {'x': x_dev.toarray()},
            y=y_dev,
            batch_size=FLAGS.batch_size,
            shuffle=False,
            num_epochs=1
        ...
        for epoch in range(self.params['num_epochs']):
            ...
            if dev_accy > max_accy:
                ...
                self.best_model_path = self.model.export_savedmodel(
                    export_dir_base=FLAGS.model_dir + "best_model/",
    ...
    def test(self, x_test, y_test, pred_test=None, program_mode='test_train'):
        """evaluate on test set using the saved model

        """

        test_accy = test_model(FLAGS.model_path, x_test,
                               y_test, pred_test, program_mode)

        print("test accy = %f%%" % (100. * test_accy))

        # write results to (same) report file in markdown format
        report_name = FLAGS.report_dir + FLAGS.report_name
        report_path = pathlib.Path(report_name)
        if not report_path.is_file():
            with open(report_name, "a") as file:
                file.write("mode|learning rate|hidden layer size|batch size"
                           "|dropout rate|l2 "
        ...
        with open(report_name, "a") as file:
            ...
            file.write(
                "best model path: " + FLAGS.model_path)
            file.write("\n")
...
def main(argv):
    ...
    if FLAGS.program_mode == 'train':
        ticks = time.clock()
        SHAPE = x_train.shape[1]
        mlp.train(x_train, y_train, x_dev, y_dev, program_mode='train')
        print("Model trained in %.6f s" % (time.clock() - ticks))

    elif FLAGS.program_mode == 'transfer':
        ticks = time.clock()
        # predict on another model and add the new features
        pred_train = predict(FLAGS.program_model_path, x=x_train)
        pred_dev = predict(FLAGS.program_model_path, x=x_dev)
        ...
        mlp.train(x_train, y_train, x_dev, y_dev, pred_train, pred_dev,
                  program_mode='transfer')
        print("New model trained in %.6f s" % (time.clock() - ticks))

    elif FLAGS.program_mode == 'test_train':
        ticks = time.clock()
        SHAPE = x_train.shape[1]
        mlp.test(x_test, y_test, program_mode='test_train')
        print("Model tested in %.6f s" % (time.clock() - ticks))

    elif FLAGS.program_mode == 'test_transfer':
        ticks = time.clock()
        # predict on another model and add the new features
        pred_test = predict(FLAGS.program_model_path, x=x_test)
        SHAPE = x_train.shape[1] + len(pred_test[0])
        mlp.test(x_test, y_test, pred_test, program_mode='test_transfer')
        print("New model tested in %.6f s" % (time.clock() - ticks))
